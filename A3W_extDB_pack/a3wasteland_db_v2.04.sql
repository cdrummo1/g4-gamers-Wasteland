...
CREATE TABLE IF NOT EXISTS `TerritoryCaptureStatus` (
  `ID` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `ServerInstance_ID` INT UNSIGNED NOT NULL,
  `ServerMap_ID` INT UNSIGNED NOT NULL,
  ...
  PRIMARY KEY (`ID`),
  INDEX `fk_TerritoryCaptureStatus_ServerMap1_idx` (`ServerMap_ID` ASC),
  INDEX `fk_TerritoryCaptureStatus_ServerInstance1_idx` (`ServerInstance_ID` ASC),
  CONSTRAINT `fk_TerritoryCaptureStatus_ServerMap1`
    FOREIGN KEY (`ServerMap_ID`)
    REFERENCES `ServerMap` (`ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_TerritoryCaptureStatus_ServerInstance1`
    FOREIGN KEY (`ServerInstance_ID`)
    REFERENCES `ServerInstance` (`ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `TerritoryCaptureLog` (
  ...
  INDEX `fk_TerritoryCaptureLog_ServerInstance1_idx` (`ServerInstance_ID` ASC),
  INDEX `fk_TerritoryCaptureLog_ServerMap1_idx` (`ServerMap_ID` ASC),
